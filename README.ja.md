...
下部 Dock の片側にパネルの設定幅を確保できない場合、その側のパネルは左右の位置と上下の順序を保ったまま Dock の上に移動します。空きスペースが戻ると、Dock の横に自動で戻ります。

週間上限はデフォルトで使用量の右側に表示されます。位置ボタンを押すと右、上、下、左の順に切り替わり、パネルの画面上の位置とは別に保存されます。上下の配置ではパネルの高さが増えます。週間上限を非表示にした場合や上限データがない場合は、追加の空間と位置ボタンが非表示になります。
