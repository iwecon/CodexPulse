...
- Preserve per-file Claude/Codex scan caches and complete-line byte cursors in memory. Unchanged files must be zero-read; continuous appends parse only the suffix after the last complete line; an older file still active at cold launch must seek to a complete-line timestamp at the window boundary instead of reading its expired prefix; truncation, replacement, new files, and removed files must rebuild or invalidate only their own entries. Prune usage records and inactive cursors outside the rolling 14-day window on every scan. Cold Codex files that start inside the window and end on a complete line use the built-in macOS `grep`/`awk` streaming prefilter; keep it shell-free, batch arguments below platform limits, verify source versions after filtering, and fall back per file for incomplete, changed, old-prefix, or failed inputs. Ordinary session content and full turn contexts must not enter the app process. `token_count` rows use the allocation-light narrow byte parser for timestamps, counters, and rate limits rather than Foundation object-tree decoding. Keep transient allocation pressure bounded during realistically large cold scans. Do not restore periodic full-history parsing.
- OpenCode scan caching must account for the SQLite database and its WAL/SHM companions so read-only caching never hides new writes. Restrict cold queries to the 14-day window and, after a source-version change, decode only rows whose IDs are new or whose `time_updated` changed; remove deleted or expired rows from memory.
