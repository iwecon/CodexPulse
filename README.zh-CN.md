...
底部 Dock 某侧的剩余空间不足以容纳面板设定宽度时，该侧面板会自动移到 Dock 上方，保留左右位置和堆叠顺序；空间恢复后自动回到 Dock 旁。

周额度默认显示在用量右侧。位置按钮按右侧、上方、下方、左侧循环切换，并单独保存选择，不受面板在屏幕上的位置影响。上方或下方布局会增加面板高度；隐藏周额度或没有额度数据时，会收起额外空间并隐藏位置按钮。
